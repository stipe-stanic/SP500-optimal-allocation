"""Templates for the two classes hosts should customize for each competition"""
...
class Gateway(kaggle_evaluation.core.base_gateway.BaseGateway, abc.ABC):
    ...
    In most case, hosts should only need to edit the three abstract methods below.

    There are two main methods for sending data to inference server hosts should understand:
    - Small datasets: use `self.predict`, which gets called by default.
    Competitors will receive the data passed to self.predict as Python objects in memory. This is just a wrapper for
    self.client.send(); you can write additional wrappers if necessary.
    - Large datasets: it's much faster to send data via self.share_files, which is equivalent to making files available
    via symlink. See base_gateway.BaseGateway.share_files for the full details
    """
...
class InferenceServer(abc.ABC):
    ...
    def _run_local_gateway(self, data_paths: Optional[Tuple[str]] = None, file_share_dir: Optional[str] = None, *args, **kwargs) -> None:
